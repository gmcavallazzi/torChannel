...
sweep uses a much longer domain ($L_x=90$ vs.\ $6$) so that all $N$ cross
$M=0.9$ within it; the threshold is therefore the \emph{same} $M=0.9$ for
every $\Sc$, an apples-to-apples comparison. The ratio
...
$N$ & {$L_{\mathrm{mix}}$} & {ratio}
    ...
    & {$L_{\mathrm{mix}}$} & {ratio} \\
\midrule
0 & 0.492 & 1.000 & 5.392 & 1.000 & 62.818 & 1.000 \\
1 & 0.303 & 0.616 & 3.821 & 0.709 & 38.370 & 0.611 \\
2 & 0.198 & 0.402 & 2.358 & 0.437 & 26.258 & 0.418 \\
3 & 0.152 & 0.309 & 1.975 & 0.366 & 22.164 & 0.353 \\
4 & 0.136 & 0.276 & 1.914 & 0.355 & 21.657 & 0.345 \\
5 & {---} & {---}  & {---} & {---}  & 21.581 & 0.344 \\
\bottomrule
\end{tabular}
...
All at the same threshold $M=0.9$; $\Sc=10,100$ use $L_x=6$, $\Sc=1000$
uses $L_x=90$ (the longer domain the high-$\Pe$ field needs to reach
$M=0.9$).}
\label{tab:lmix}
\end{table}
...
on the chosen $M$. ($\Sc=100$ spans only a narrow band here because its
$L_x=6$ domain mixes to just $M\!\approx\!0.84$; a longer-domain run widens
it to match the $\Sc=10$ range.)}
\label{fig:thresh}
\end{figure}
...
($L_x=6$, left) and $\Sc=1000$ ($L_x=90$, right). Higher $N$ decays faster
...
\item \textbf{The fractal interface robustly shortens the mixing length,
  ...
  to $N=3$ cuts $L_{\mathrm{mix}}$ to $0.31$, $0.37$ and $0.35$ of the flat
...
\item \textbf{$L_{\mathrm{mix}}$ scales steeply with $\Pe$.}
  The $N=0$ baseline grows $0.49\!\to\!5.4\!\to\!63$ for
