...
def panel(ax, indir, Sc, kind, Ns, Mlo, Mhi, ncol=1):
...
def main():
    ...
    # main figure: Sc=10 and Sc=100 over a wide, common threshold range
    ...
    # side figure: Sc=1000 (narrower reachable band -- the Lx=90 field mixes
    # N=0 only to M~0.88, so the ratio is defined only for M >~ 0.88)
    fig2, ax2 = plt.subplots(1, 1, figsize=(5.4, 4.4))
    panel(ax2, a.indir, 1000, 'march', [1, 2, 3, 4, 5], 0.882, 0.925, ncol=2)
    ax2.set_ylabel(r"$L_{\mathrm{mix}}(N)/L_{\mathrm{mix}}(0)$")
    fig2.tight_layout()
    o2 = os.path.join(a.outdir, "baffle_ratio_vs_threshold_Sc1000.png")
    fig2.savefig(o2, dpi=150, bbox_inches='tight'); print("wrote", o2)
