...
- **Reynolds stresses: three conventions that all bias `u'` low and `w'` not at all**, so getting any of them wrong looks exactly like numerical dissipation acting on the streamwise component, or like wall-parallel under-resolution. All three cost ~0 to do right:
  1. **About the TIME mean, not the instantaneous plane mean.** `<(u-<U>)^2> = <(u-U(t))^2> + var_t(U(z))`; the second term must be accumulated (`Usq_sum` etc.) or `u'u'` is low by it. Invisible in `w'w'` and `<u'w'>`: continuity + periodicity pin the plane mean of `w` to zero identically (verified `|W| < 4e-16`) — unless a wall velocity is imposed, which is why the `W`/`UW` terms are kept.
  2. **At each component's OWN node, not the cell centre.** The 2-point average to the cell centre is a low-pass filter, `cos(k*d/2)`, removing variance in proportion to the spacing in *that component's* staggered direction — `dx` for u, `dy` for v, `dz` for w. `u` and `v` already sit at z-centres, so no interpolation is needed at all. `w'w'` is formed at z-faces and the *profile* interpolated (statistic, not field). Matters most for the **spectra**: the filter multiplies `E_uu(kx)` by `cos^2(kx*dx/2)`, exactly zero at the cutoff — a manufactured high-k roll-off in the very diagnostic used to judge resolution.
  3. **Fold a closed channel about the centreline** in post-processing (`<u'w'>` odd, the rest even; before `compute_dUdz`). Free 2x samples.
  `<u'w'>` needs co-location whatever you do, and is insensitive to the choice (two natural co-locations agree to 0.00%). CaNS agrees on 1 and 2 (`out1d_chan`, `output.f90`); it defers 1 to post-processing by writing `um` and `u2` per snapshot, so naively time-averaging its `u2` column reproduces the bias.
- **Bulk forcing is an exact one-step constraint, not a controller.** `_apply_bulk_forcing` shifts `u` uniformly so `u_bulk` lands on target (divergence-free, so it commutes with the projection) and *diagnoses* the force. It is NOT added to the momentum RHS — doing so puts it through AB2 as `1.5f_n - 0.5f_{n-1}`, extrapolating a control signal. The `0.1/dt` integral controller it replaced was an undamped oscillator (period ~20 steps) whose gain, and therefore closed-loop response, scaled with dt — which denies the scheme any clean temporal order and silently contaminates dt sweeps.
- **`solver.incremental_pressure` (default true) does not change the velocity.** Because the Poisson operator is built with `L = D·G` exactly, `p' + p^n = p`, so incremental and non-incremental projection give identical `u^{n+1}` up to the implicit-diffusion splitting (~1e-3 relative). It changes what `self.p` *means* — the accumulated physical pressure rather than a one-step potential. Do not expect it to move statistics.
